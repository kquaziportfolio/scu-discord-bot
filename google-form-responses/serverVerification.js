    ...
    app.post("/verify", (req, res) => {
      ...
      if (req.headers["key"] != config.verification.key) {
        ...
        if (member == null) {
          ...
          guild.channels.cache.get(config.channels.welcome).send(`**<@${member.user.id}>**`, { embed: { title: `__**✅ NEW VERIFIED MEMBER!**__`, description: `You are now verified! Everyone please welcome **${req.body.name}** to the server!`, color: config.school_color, timestamp: new Date()}}).then(m => m.react('👋'));
