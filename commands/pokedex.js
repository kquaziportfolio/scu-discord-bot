...
module.exports = {
	...
	async execute(message, args) {
        ...
        message.channel.send("Fetching from the Pokedex...").then(msg => {
			...
			const options = {
				url: `https://courses.cs.washington.edu/courses/cse154/webservices/pokedex/pokedex.php?pokemon=${name}`,
				json: true
			}
			...
			let embed = new MessageEmbed()
			  ...
			  .catch(err => {
				  message.channel.send({embed: {description: `${err}`, color: 10231598, timestamp: new Date()}}) 
			  })
			}).catch(err => {
				message.channel.send({embed: {description: `${err}`, color: 10231598, timestamp: new Date()}}) 
			})
