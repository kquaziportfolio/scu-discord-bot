...
module.exports  = {
    ...
    async execute(client, message, args) {
        ...
        if (body.status.description == "All Systems Operational") {
            ...
            message.channel.send({ embed: { title: `:x: ${body.status.description}`, description: `There seems to be an error with some of the Discord servers. Double check [here](https://status.${args[0]}.com/api/v2/status.json)! :x:`, color: "RED", timestamp: new Date()}});
        }
    }
}