...
client.on(`messageReactionAdd`, async (reaction, user) => {
  ...
  if (reaction.message.channel.id === "722494512420618370") {
    ...
    if (reaction.emoji.name === "☣️") {
      ...
      return user
        .send({ embed: { description: `<@${user.id}>, ` + "`Computer Science and Engineering (SOE)` role was added!", timestamp: new Date(), footer: { text: "Go Broncos!" }, color: 10231598 } })
...
client.on(`messageReactionRemove`, async (reaction, user) => {
  ...
  if (reaction.message.channel.id === "722494512420618370") {
    if (reaction.emoji.name === "☣️") {
      ...
      return user
        .send({ embed: { description: `<@${user.id}>, ` + "`Computer Science and Engineering (SOE)` role was removed!", timestamp: new Date(), footer: { text: "Go Broncos!" }, color: 10231598 } })
        .catch(() => console.log("Failed to send DM."));
    } else if (reaction.emoji.name === "🏗️") {
      ...
      message.channel
        .send({
          embed: {
            ...
            description: `If you think this is an infringement on your speech, please contact <@&709118762707845211>/<@&710593727864897646> right away.` + ` Otherwise, rephrase your speech so the bot doesn't think you're using the word!`,
            color: 10231598,
            thumbnail: {
