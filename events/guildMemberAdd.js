...
module.exports = async (client, member, message) => { 
    ...
    guild.systemChannel.send(new MessageEmbed() // triggers when new users joins to specific channel in server
        .setTitle(`Welcome to the **${guild.name}**!`) // Calling method setTitle on constructor. 
        .setDescription(`<@${memberTag}> has joined **${guild.name}**! ` + 
        `Be sure to follow instructions in the DM! Go Broncos!`) //Setting embed description
    ...
    const welcome_Embed = new MessageEmbed() //personal message to new user
        ...
        `${emojiCharacters.two} Fill out the Google Form [here](https://forms.gle/vqmrDx9LRVexdwLk6) to verify yourself in the SCU server! It'll only take a couple seconds! Note: If you're a guest or alumni, you are exempted from this requirement \n\n` +
